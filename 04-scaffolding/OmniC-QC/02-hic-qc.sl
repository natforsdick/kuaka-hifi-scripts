...
#SBATCH -A ga03186
#SBATCH -J hic_qc 
#SBATCH -c 24 # hicqc only needs 2 cpu
#SBATCH --mem=15G # hicqc requires <500M 
#SBATCH --partition=large 
#SBATCH --time=00:05:00 #hicqc only takes <5 min 
...
cd ${IN_DIR}
if [ ! -e ${IN_DIR}${IN_BAM}*.bai ]; then
	echo "Sorting ${HIC}"
	samtools sort -@ $CPU ${IN_DIR}${IN_BAM}.bam -o ${IN_DIR}${IN_BAM}-sorted.bam
else
	echo "sortde BAM found, running QC"
fi


cd $IN_DIR
echo "running hic_qc for ${IN_BAM}"
python ${hic_qc} -b ${IN_BAM}.bam -o ${IN_BAM}.hicqc 
